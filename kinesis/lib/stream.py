...
class Stream(object):
    def __init__(self):
        ...
        self.stream_name = 'cis-prototype' #Fix this later to come from config
    ...
    def __boto_session(self):
        session = boto3.Session(profile_name='cis-dev') #Fix later to know if prod or dev
        return session
...
class Subscribe(Stream):
    ...
    def shard_iterator(self):
        ...
        response = self.client.get_shard_iterator(
            ...
            ShardIteratorType='TRIM_HORIZON' #For now return all records can be more elegant later
